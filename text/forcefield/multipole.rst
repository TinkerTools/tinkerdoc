...
.. math::
   ...
   \phi(r)=\frac{1}{4\pi\epsilon_0}\int ds\frac{\rho(s)}{|r-s|},\ ds=dxdydz.

Tinker uses a variable **electric** (defined in **chgpot** module) to represent the factor :math:`1/(4\pi\epsilon_0)`. Its default magnitude is 332.063713, which is a constant defined by variable **coulomb** (in **units** module), and its units are kcal/mol |ang|/|e2|. The default value can be modified by the *ELECTIRIC* keyword.
...
Expanding :math:`1/|r-s|` in Taylor series, :math:`4\pi\epsilon_0\phi(r)` can be rewritten as
...
In addition to different units, the quadrupole moments in Tinker parameter files use what is traditionally called *traceless quadrupole* :math:`\Theta` that has a different definition than *Q\**. The third term in :eq:`pot2` can be rewritten as
...
Since :math:`\sum_{k=x,y,z}(3 s_k s_k - s^2)=0`,

.. math::

   \Theta_{ij} = 3Q_{ij}^* - \delta_{ij}\sum_{k=x,y,z}Q_{kk}^*.
...
In the EWALD summation, :math:`1/r^k` terms take different forms (*Bn*). Neverthelss, they are still connected through derivatives.
...
The *Bn* terms are related to the (complementary) Boys functions and (complementary) error functions. For :math:`x>0` and :math:`n\ge 0`,
