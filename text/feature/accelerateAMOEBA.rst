...
Cutoff distances.
   ...
   For a partial charge force field, the performance of Tinker9 is no longer dominated by electrostatics. VDW and partial charge PME become comparable, or VDW may even become dominant if the VDW cutoff is significantly longer than the PME cutoff due to the rapid growth of the number of interactions. Therefore, it is a common practice for some partial charge force fields to set two cutoffs to the same distance, e.g., 9 |ang|. Tinker9 will launch an optimized CUDA kernel that combines the Lennard-Jones and partial charge potentials if it detects that two cutoffs are equal.

   .. seealso::

      :ref:`EWALD <KEY-EWALD>`, :ref:`EWALD-CUTOFF <KEY-EWALD-CUTOFF>`,
      :ref:`VDW-CUTOFF <KEY-VDW-CUTOFF>`, :ref:`VDW-CORRECTION <KEY-VDW-CORRECTION>`

Neighbor list and list buffer.
   Using neighbor lists, Tinker9 will prioritize the use of CUDA kernels with optimized spatial decomposition algorithms, instead of the classic Verlet Lists. The trade-off of using a longer list buffer is that the reconstruction or update of the neighbor list is less frequent but the possible number of pairwise interactions is greater. Since the default value of the list buffer in Tinker9 is inherited from the canonical Tinker, it may not be optimized for Tinker9's spatial decomposition algorithms. The optimized length of the list buffer varies with different systems and can be approximately determined by benchmarking a few short simulations.

   .. seealso::

      :ref:`NEIGHBOR-LIST <KEY-NEIGHBOR-LIST>`, :ref:`LIST-BUFFER <KEY-LIST-BUFFER>`
...
Induced dipoles.
   ...
   .. seealso::
      ...
      :ref:`POLAR-EPS <KEY-POLAR-EPS>`, :ref:`POLAR-PREDICT <KEY-POLAR-PREDICT>`
