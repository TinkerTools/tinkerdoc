...
Tinker9 implements the Langevin Piston integrator in two ways: a single time-step integrator similar to Velocity-Verlet and a multiple time-step integrator similar to RESPA. They can be employed by the following keywords, respectively. The thermostat method is built in the integrator, thus the thermostat keyword is ignored.
